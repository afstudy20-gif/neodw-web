...
export interface AortaPanelHandle {
  saveSession: () => void;
      ...
      {visible && hasMask && data && dims && voxelToWorld ? (
        <>
          ...
          <div style={{ position: 'absolute', top: 6, right: 6, zIndex: 60, display: 'flex', gap: 4 }}>
            <button
              onClick={onRebuild}
              title="Rebuild aorta mesh from current mask"
              style={{
        ...
        <div style={{
          ...
          {hasMask ? (
            ...
            <div>
              Aorta 3D mesh will appear here.<br />
...
const AO_SEGMENTATION_ID = 'aortaSegmentation';
const AO_EXCLUDE_SEGMENTATION_ID = 'aortaExcludeMask';
...
const DEFAULT_MIN_HU = 300;
const DEFAULT_MAX_HU = 500;
const AO_VOXEL_CAP = 1_200_000; // Aorta typical volume ≈150–300 mL at 1mm isotropic
...
export const AortaPanel = forwardRef<AortaPanelHandle, Props>(function AortaPanel(
  ...
  const [meshBg, setMeshBg] = useState<[number, number, number]>([0.0, 0.0, 0.0]);
  const [meshAlpha, setMeshAlpha] = useState(0.95);
  ...
  const clearSegmentation = useCallback(() => {
    ...
    for (const vpId of ALL_VIEWPORT_IDS) {
      try {
        segmentation.removeSegmentationRepresentations(vpId, {
          segmentationId: AO_SEGMENTATION_ID,
        });
      } catch { /* ignore */ }
    }
    try { segmentation.removeSegmentation(AO_SEGMENTATION_ID); } catch { /* ignore */ }
    ...
    if (excludeLabelmapIdRef.current) {
      for (const vpId of MPR_VIEWPORT_IDS) {
        try { segmentation.removeSegmentationRepresentations(vpId, { segmentationId: AO_EXCLUDE_SEGMENTATION_ID }); } catch { /* ignore */ }
      }
      try { segmentation.removeSegmentation(AO_EXCLUDE_SEGMENTATION_ID); } catch { /* ignore */ }
  ...
  const syncExcludeVisualization = useCallback(async () => {
    ...
    if (!excludeLabelmapIdRef.current) {
      ...
      segmentation.addSegmentations([
        {
          segmentationId: AO_EXCLUDE_SEGMENTATION_ID,
          representation: {
            type: ToolsEnums.SegmentationRepresentations.Labelmap,
            data: { volumeId: id },
          },
      ...
      if (lm && arr) {
        ...
        try {
          (cornerstoneTools as any).segmentation?.triggerSegmentationEvents
            ?.triggerSegmentationDataModified?.(AO_EXCLUDE_SEGMENTATION_ID);
        } catch { /* ignore */ }
      }
    }
  }, [volumeId]);
  ...
  const clearExcludeVisualization = useCallback(() => {
    ...
    if (excludeLabelmapIdRef.current) {
      for (const vpId of MPR_VIEWPORT_IDS) {
        try {
          segmentation.removeSegmentationRepresentations(vpId, { segmentationId: AO_EXCLUDE_SEGMENTATION_ID });
        } catch { /* ignore */ }
      }
      try { segmentation.removeSegmentation(AO_EXCLUDE_SEGMENTATION_ID); } catch { /* ignore */ }
      try { cornerstone.cache.removeVolumeLoadObject(excludeLabelmapIdRef.current); } catch { /* ignore */ }
  ...
  const applyData = useCallback(async (newData: Uint8Array, opts?: { skipUndo?: boolean }) => {
    ...
    if (lm && lmArr) {
      ...
      try {
        (cornerstoneTools as any).segmentation?.triggerSegmentationEvents
          ?.triggerSegmentationDataModified?.(AO_SEGMENTATION_ID);
      } catch { /* best-effort */ }
    } else {
  ...
  useEffect(() => {
    ...
    for (const vpId of MPR_VIEWPORT_IDS) {
      ...
      overlay.title = seedMode
        ? 'Click to place aorta seed'
        : mvMode
          ? 'Click to add MV plane point'
          : editMode === 'paint'
            ? 'Drag to paint aorta voxels'
            : 'Drag to erase aorta voxels';
      ...
      const paintAt = (clientX: number, clientY: number) => {
        const cur = laStateRef.current;
        if (!cur) { console.warn('[Aorta Paint] no mask — run Flood-Fill first'); return; }
        const rect = el.getBoundingClientRect();
        const cx = clientX - rect.left;
        const cy = clientY - rect.top;
  ...
  const saveSession = useCallback(() => {
    ...
    const fname = buildSessionFilename(patientName, studyDate, 'AO');
  ...
  const runReconstruction = useCallback(async () => {
    ...
    setStatusMsg(null);
    if (!seedWorld) {
      setError('Place seed inside aorta first (lumen, any level).');
      return;
    }
  ...
  const runExportSTL = useCallback(async () => {
    ...
    try {
      ...
      const blob = meshToBinarySTL(mesh, 'Aorta — antidicom');
  ...
  return (
    <div className="la-panel">
      ...
      <div className="la-section">
        ...
        <div className="la-preset-row">
          <button
            ...
            disabled={running}
            title="Click mid-chest axial slice inside aorta lumen (root, arch, or descending). HU target ≈300–400."
          >
          ...
          <button
            ...
            disabled={running}
            title="Add extra seeds for disconnected branches / arch vessels (multi-seed merge)."
      ...
      {hasMask && (
        <>
          <div className="la-section">
            <h4>4. Clean-up (keep aorta trunk)</h4>
            <p className="la-hint">
              <strong>Goal:</strong> keep aorta + root + branches of interest. Remove LV (proximal cut below annulus), unwanted descending/iliac (distal cut), spine/coronaries/IVC bleeds (Erase brush).
            </p>
            <div className="la-preset-row">
              <button className="la-btn la-btn-secondary" onClick={async () => {
                ...
                <span className="la-btn-sub">keep largest blob (aorta trunk)</span>
              </button>
              <button className="la-btn la-btn-secondary" onClick={async () => {
                const cur = laStateRef.current; if (!cur) return;
                setRunning(true);
            ...
            <div className="la-preset-row" style={{ marginTop: 8 }}>
              ...
              }} disabled={running} title="Morphological erosion (ball r=1). Breaks thin bridges between aorta and neighbouring structures (IVC, pulmonary artery). Follow with Drop Fragments, then Grow 1 vox.">
          ...
          <div className="la-section">
            <h4>5. Proximal Cut (below aortic valve)</h4>
            <p className="la-hint">
              Click 3 points on the aortic annulus (supra-ventricular side). Plane removes LV and non-aortic tissue; aorta side preserved (seed-anchored).
            </p>
            ...
            >
              {mvMode
                ? `Click point ${mvPoints.length + 1}/3 on aortic annulus…`
                : mvPoints.length >= 3
                  ? 'Re-pick MV Points'
          ...
          <div className="la-section">
            <h4>5b. Distal Cut (descending / iliac / arch)</h4>
            <p className="la-hint">
              Click 3 points on the unwanted distal aorta slice (descending below diaphragm, iliac bifurcation, or non-target arch branch). Plane removes that side; trunk kept (seed-anchored).
            </p>
            <p className="la-hint" style={{ color: '#ffcc66' }}>
              <strong>Pick across 3 different slices</strong> (same rule as MV). All 3 on one axial → nearly horizontal plane → wrong cut.<br />
              Pick on the distal side of the aorta section you want removed (below diaphragm, iliac, or unwanted branch). Keep points across 3 slices.<br />
            ...
            >
              {aortaMode
                ? `Click point ${aortaPoints.length + 1}/3 on distal cut line…`
                : aortaPoints.length >= 3
                  ? 'Re-pick Distal Points'
