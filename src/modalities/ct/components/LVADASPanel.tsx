...
import { computeWallThickness, lvThicknessToColor, computeFatFraction, fatFractionToColor } from '../la/wallThickness';
import { computeBZCorridors, buildCorridorColors, type CorridorResult } from '../la/corridors';
import { computeLongAxisFromMV, computeLongAxisPCA, computeSegmentStats, type SegStat } from '../la/aha17';
import { Bullseye17 } from './Bullseye17';
import {
  encodeBinaryRLE,
  decodeBinaryRLE,
...
export interface LVADASPanelHandle {
  saveSession: () => void;
...
function LA3DInlineView({
  ...
  onMesh: (mesh: import('../la/marchingCubes').Mesh) => void;
  onComputeWallMap: () => void;
  onClearWallMap: () => void;
  wallStats: { minMm: number; maxMm: number; meanMm: number; measurable: number; total: number } | null;
  computingWall: boolean;
  layerDepth: number;
  setLayerDepth: (d: number) => void;
  colorMode: 'thickness' | 'fat' | 'corridor';
  setColorMode: (m: 'thickness' | 'fat' | 'corridor') => void;
  fatStats: { meanPct: number; maxPct: number; vertexWithFatPct: number } | null;
    ...
    >
      {visible && hasMask && data && dims && voxelToWorld ? (
        <>
          ...
          />
          <div style={{ position: 'absolute', top: 40, right: 6, zIndex: 60, display: 'flex', gap: 4 }}>
            <button
              onClick={vertexColors ? onClearWallMap : onComputeWallMap}
              disabled={computingWall}
              title={vertexColors ? 'Clear wall-thickness coloring' : 'Ray-cast wall thickness from endocardium outward; color-code scar proxy'}
              style={{
                padding: '4px 10px', fontSize: 11, fontWeight: 600,
                background: vertexColors ? 'rgba(60,140,60,0.45)' : 'rgba(230,160,60,0.5)',
                border: vertexColors ? '1px solid rgba(60,200,60,0.75)' : '1px solid rgba(230,160,60,0.85)',
                color: '#fff', borderRadius: 4, cursor: computingWall ? 'wait' : 'pointer',
                opacity: computingWall ? 0.6 : 1,
              }}
            >{computingWall ? '…computing' : vertexColors ? '✓ Wall Map' : 'Wall Map'}</button>
            <button
              onClick={onRebuild}
              title="Rebuild LV mesh from current mask"
              style={{
                padding: '4px 10px', fontSize: 11, fontWeight: 600,
          ...
          {vertexColors && wallStats ? (
            <div style={{
              position: 'absolute', top: 6, left: 6, zIndex: 60,
              background: 'rgba(0,0,0,0.62)', padding: '6px 8px', borderRadius: 6,
              color: '#fff', fontSize: 10.5, lineHeight: 1.35,
              display: 'flex', flexDirection: 'column', gap: 4, minWidth: 140,
            }}>
              <div style={{ display: 'flex', gap: 3, marginBottom: 2 }}>
                <button
                  onClick={() => setColorMode('thickness')}
                  style={{
                    flex: 1, padding: '2px 4px', fontSize: 9.5, borderRadius: 3,
                    background: colorMode === 'thickness' ? 'rgba(121,199,255,0.55)' : 'rgba(255,255,255,0.1)',
                    border: '1px solid rgba(255,255,255,0.25)', color: '#fff', cursor: 'pointer', fontWeight: 600,
                  }}
                >Thickness</button>
                <button
                  onClick={() => setColorMode('fat')}
                  disabled={!fatStats}
                  style={{
                    flex: 1, padding: '2px 4px', fontSize: 9.5, borderRadius: 3,
                    background: colorMode === 'fat' ? 'rgba(230,200,60,0.55)' : 'rgba(255,255,255,0.1)',
                    border: '1px solid rgba(255,255,255,0.25)', color: '#fff',
                    cursor: fatStats ? 'pointer' : 'not-allowed', fontWeight: 600, opacity: fatStats ? 1 : 0.5,
                  }}
                >inFAT</button>
                <button
                  onClick={() => setColorMode('corridor')}
                  disabled={!corridorResult || corridorResult.corridors.length === 0}
                  style={{
                    flex: 1, padding: '2px 4px', fontSize: 9.5, borderRadius: 3,
                    background: colorMode === 'corridor' ? 'rgba(120,220,240,0.55)' : 'rgba(255,255,255,0.1)',
                    border: '1px solid rgba(255,255,255,0.25)', color: '#fff',
                    cursor: corridorResult && corridorResult.corridors.length > 0 ? 'pointer' : 'not-allowed',
                    fontWeight: 600,
                    opacity: corridorResult && corridorResult.corridors.length > 0 ? 1 : 0.5,
                  }}
                >Corridors</button>
              </div>
              {colorMode === 'corridor' ? (
                <>
                  <div style={{ fontWeight: 700, fontSize: 11 }}>BZ Corridors (VT channels)</div>
                  {corridorResult && corridorResult.corridors.length > 0 ? (
                    <>
                      <div style={{ fontSize: 10, opacity: 0.9 }}>
                        {corridorResult.corridors.length} corridor{corridorResult.corridors.length > 1 ? 's' : ''} · core {corridorResult.coreCount.toLocaleString()} · BZ {corridorResult.bzCount.toLocaleString()}
                      </div>
                      <div style={{ maxHeight: 80, overflowY: 'auto', fontSize: 9.5 }}>
                        {corridorResult.corridors.slice(0, 8).map((c, i) => (
                          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', gap: 6 }}>
                            <span>#{i + 1}</span>
                            <span>~{c.lengthMm.toFixed(0)}mm</span>
                            <span>{c.size}v</span>
                          </div>
                        ))}
                      </div>
                    ...
                    <div style={{ fontSize: 10, opacity: 0.7 }}>No corridors found (3-5mm BZ bounded by scar + healthy).</div>
                  )}
                  <div style={{ fontSize: 9.5, opacity: 0.7, marginTop: 2 }}>
                    distinct color per corridor · bounded by core &lt;3mm + healthy &gt;5mm
                  </div>
                </>
              ) : colorMode === 'thickness' ? (
                <>
                  <div style={{ fontWeight: 700, fontSize: 11 }}>LV Wall Thickness (mm)</div>
                  <div style={{
                    height: 12, borderRadius: 2,
                    background: 'linear-gradient(to right, rgb(180,12,12) 0%, rgb(242,50,38) 20%, rgb(242,165,38) 40%, rgb(242,230,50) 55%, rgb(90,220,100) 80%, rgb(50,90,200) 100%)',
                  }} />
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 9.5, opacity: 0.85 }}>
                    <span>0</span><span>3</span><span>5</span><span>7</span><span>10</span><span>15+</span>
                  </div>
                  <div style={{ fontSize: 10, opacity: 0.9 }}>
                    min {wallStats.minMm.toFixed(1)} · mean {wallStats.meanMm.toFixed(1)} · max {wallStats.maxMm.toFixed(1)}
                  </div>
                  <div style={{ fontSize: 9.5, opacity: 0.75 }}>
                    {wallStats.measurable.toLocaleString()} / {wallStats.total.toLocaleString()} vertices
                  </div>
                  <div style={{ fontSize: 9.5, opacity: 0.7, marginTop: 2 }}>
                    red = scar · yellow = channel · green = healthy
                  </div>
                </>
              ) : (
                <>
                  <div style={{ fontWeight: 700, fontSize: 11 }}>Intramyocardial Fat (HU &lt; −30)</div>
                  <div style={{
                    height: 12, borderRadius: 2,
                    background: 'linear-gradient(to right, rgb(140,140,50) 0%, rgb(200,180,40) 50%, rgb(255,230,0) 100%)',
                  }} />
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 9.5, opacity: 0.85 }}>
                  ...
                  {fatStats && (
                    <>
                      <div style={{ fontSize: 10, opacity: 0.9 }}>
                        mean {fatStats.meanPct.toFixed(1)}% · max {fatStats.maxPct.toFixed(1)}%
                      </div>
                      <div style={{ fontSize: 9.5, opacity: 0.75 }}>
                        {fatStats.vertexWithFatPct.toFixed(1)}% of vertices show fat &gt;5%
                      </div>
                    </>
                  )}
                  <div style={{ fontSize: 9.5, opacity: 0.7, marginTop: 2 }}>
                    yellow = fat · ARVC / fibrofatty substrate
                  </div>
                </>
              )}
              <div style={{ borderTop: '1px solid rgba(255,255,255,0.15)', paddingTop: 5, marginTop: 3 }}>
                <div style={{ fontSize: 10, fontWeight: 700, marginBottom: 2 }}>Transmural Layer</div>
                <div style={{ display: 'flex', gap: 3, marginBottom: 3 }}>
                  {[
                    ...
                    <button
                      key={p.label}
                      onClick={() => setLayerDepth(p.d)}
                      style={{
                        padding: '2px 5px', fontSize: 9, borderRadius: 3,
                        background: Math.abs(layerDepth - p.d) < 0.05 ? 'rgba(121,199,255,0.55)' : 'rgba(255,255,255,0.1)',
                ...
                <div style={{ fontSize: 9.5, opacity: 0.85 }}>
                  depth {Math.round(layerDepth * 100)}% · 0%=endo, 100%=epi
                </div>
              </div>
            </div>
          ) : null}
        ...
        <div style={{
          ...
          {hasMask ? (
            ...
            <div>
              LV-ADAS 3D wall map will appear here.<br />
...
const LV_SEGMENTATION_ID = 'lvSegmentation';
const LV_EXCLUDE_SEGMENTATION_ID = 'lvExcludeMask';
...
export const LVADASPanel = forwardRef<LVADASPanelHandle, Props>(function LVADASPanel(
  ...
  const [wallPositions, setWallPositions] = useState<Float32Array | null>(null);
  const [wallStats, setWallStats] = useState<{ minMm: number; maxMm: number; meanMm: number; measurable: number; total: number } | null>(null);
  const [wallThicknessArr, setWallThicknessArr] = useState<Float32Array | null>(null);
  const [wallFatArr, setWallFatArr] = useState<Float32Array | null>(null);
  const [fatStats, setFatStats] = useState<{ meanPct: number; maxPct: number; vertexWithFatPct: number } | null>(null);
  const [colorMode, setColorMode] = useState<'thickness' | 'fat' | 'corridor'>('thickness');
  const [corridorResult, setCorridorResult] = useState<CorridorResult | null>(null);
  const [layerDepth, setLayerDepth] = useState(1.0); // 0.1=subendo ... 1.0=full epi
  const [computingWall, setComputingWall] = useState(false);
  const [segStats, setSegStats] = useState<SegStat[] | null>(null);
  const currentMeshRef = useRef<Mesh | null>(null);

  // Rebuild vertex colors when colorMode / thickness / fat / corridors change
  useEffect(() => {
    if (!wallThicknessArr) return;
    const n = wallThicknessArr.length;
    if (colorMode === 'corridor' && corridorResult) {
      setWallColors(buildCorridorColors(n, corridorResult.corridors));
      return;
    }
    const colors = new Float32Array(n * 3);
    if (colorMode === 'fat' && wallFatArr && wallFatArr.length === n) {
      for (let v = 0; v < n; v++) {
        const f = wallFatArr[v];
        const c0 = v * 3;
        const [r, g, b] = fatFractionToColor(f);
        colors[c0] = r; colors[c0 + 1] = g; colors[c0 + 2] = b;
      }
    } else {
      for (let v = 0; v < n; v++) {
        const t = wallThicknessArr[v];
        const c0 = v * 3;
        if (Number.isNaN(t)) { colors[c0] = 0.4; colors[c0 + 1] = 0.4; colors[c0 + 2] = 0.4; }
        else {
          const [r, g, b] = lvThicknessToColor(t);
          colors[c0] = r; colors[c0 + 1] = g; colors[c0 + 2] = b;
        }
      }
    }
    setWallColors(colors);
  }, [colorMode, wallThicknessArr, wallFatArr, corridorResult]);

  // Rebuild displaced positions when layerDepth or thickness changes
  useEffect(() => {
    if (!wallThicknessArr) return;
    const mesh = currentMeshRef.current;
    if (!mesh) return;
    const n = mesh.triangleCount * 3;
    if (wallThicknessArr.length !== n) return;
    let meanT = 0, cnt = 0;
    for (let v = 0; v < n; v++) {
      const t = wallThicknessArr[v];
      if (!Number.isNaN(t)) { meanT += t; cnt++; }
    }
    meanT = cnt > 0 ? meanT / cnt : 5;
    const displaced = new Float32Array(mesh.positions.length);
    for (let v = 0; v < n; v++) {
      const t = wallThicknessArr[v];
      const off = (Number.isNaN(t) ? meanT : t) * layerDepth;
      const p0 = v * 3;
      displaced[p0]     = mesh.positions[p0]     + mesh.normals[p0]     * off;
      displaced[p0 + 1] = mesh.positions[p0 + 1] + mesh.normals[p0 + 1] * off;
      displaced[p0 + 2] = mesh.positions[p0 + 2] + mesh.normals[p0 + 2] * off;
    }
    setWallPositions(displaced);
  }, [layerDepth, wallThicknessArr]);
  ...
  const clearSegmentation = useCallback(() => {
    ...
    for (const vpId of ALL_VIEWPORT_IDS) {
      try {
        segmentation.removeSegmentationRepresentations(vpId, {
          segmentationId: LV_SEGMENTATION_ID,
        });
      } catch { /* ignore */ }
    }
    try { segmentation.removeSegmentation(LV_SEGMENTATION_ID); } catch { /* ignore */ }
    ...
    if (excludeLabelmapIdRef.current) {
      for (const vpId of MPR_VIEWPORT_IDS) {
        try { segmentation.removeSegmentationRepresentations(vpId, { segmentationId: LV_EXCLUDE_SEGMENTATION_ID }); } catch { /* ignore */ }
      }
      try { segmentation.removeSegmentation(LV_EXCLUDE_SEGMENTATION_ID); } catch { /* ignore */ }
  ...
  const syncExcludeVisualization = useCallback(async () => {
    ...
    if (!excludeLabelmapIdRef.current) {
      ...
      segmentation.addSegmentations([
        {
          segmentationId: LV_EXCLUDE_SEGMENTATION_ID,
          representation: {
            type: ToolsEnums.SegmentationRepresentations.Labelmap,
            data: { volumeId: id },
          },
      ...
      if (lm && arr) {
        ...
        try {
          (cornerstoneTools as any).segmentation?.triggerSegmentationEvents
            ?.triggerSegmentationDataModified?.(LV_EXCLUDE_SEGMENTATION_ID);
        } catch { /* ignore */ }
      }
    }
  }, [volumeId]);
  ...
  const clearExcludeVisualization = useCallback(() => {
    ...
    if (excludeLabelmapIdRef.current) {
      for (const vpId of MPR_VIEWPORT_IDS) {
        try {
          segmentation.removeSegmentationRepresentations(vpId, { segmentationId: LV_EXCLUDE_SEGMENTATION_ID });
        } catch { /* ignore */ }
      }
      try { segmentation.removeSegmentation(LV_EXCLUDE_SEGMENTATION_ID); } catch { /* ignore */ }
      try { cornerstone.cache.removeVolumeLoadObject(excludeLabelmapIdRef.current); } catch { /* ignore */ }
  ...
  const applyData = useCallback(async (newData: Uint8Array, opts?: { skipUndo?: boolean }) => {
    ...
    if (lm && lmArr) {
      ...
      try {
        (cornerstoneTools as any).segmentation?.triggerSegmentationEvents
          ?.triggerSegmentationDataModified?.(LV_SEGMENTATION_ID);
      } catch { /* best-effort */ }
    } else {
  ...
  useEffect(() => {
    ...
    for (const vpId of MPR_VIEWPORT_IDS) {
      ...
      overlay.title = seedMode
        ? 'Click to place LV seed'
        : mvMode
          ? 'Click to add MV plane point'
          : editMode === 'paint'
            ? 'Drag to paint LV voxels'
            : 'Drag to erase LV voxels';
      ...
      const paintAt = (clientX: number, clientY: number) => {
        const cur = laStateRef.current;
        if (!cur) { console.warn('[LV Paint] no mask — run Flood-Fill first'); return; }
        const rect = el.getBoundingClientRect();
        const cx = clientX - rect.left;
        const cy = clientY - rect.top;
  ...
  const saveSession = useCallback(() => {
    ...
    const fname = buildSessionFilename(patientName, studyDate, 'LV');
  ...
  const runReconstruction = useCallback(async () => {
    ...
    setStatusMsg(null);
    if (!seedWorld) {
      setError('Place seed inside LV cavity first.');
      return;
    }
  ...
  const runExportSTL = useCallback(async () => {
    ...
    try {
      ...
      const blob = meshToBinarySTL(mesh, 'LV-ADAS — antidicom');
  ...
  return (
    <div className="la-panel">
      <div className="la-section">
        <h4>LV-ADAS (Wall Thickness Map)</h4>
        <p className="la-hint">
      ...
      <div className="la-section">
        ...
        <div className="la-preset-row">
          <button
            ...
            disabled={running}
            title="Click mid-LV axial slice inside blood pool (between papillaries, not on endocardial trabeculae). HU target ≈350–450."
          >
          ...
          <button
            ...
            disabled={running}
            title="Add extra seeds inside LV cavity if flood-fill misses apex or basal regions due to trabeculae bridging."
      ...
      <LA3DInlineView
        ...
        onRebuild={() => { setWallColors(null); setWallPositions(null); setWallStats(null); setSegStats(null); setWallThicknessArr(null); setWallFatArr(null); setFatStats(null); setCorridorResult(null); setMeshTick((t) => t + 1); }}
        meshColor={meshColor}
        setMeshColor={setMeshColor}
        meshBg={meshBg}
        setMeshBg={setMeshBg}
        meshAlpha={meshAlpha}
        setMeshAlpha={setMeshAlpha}
        vertexColors={wallColors}
        displacedPositions={wallPositions}
        onMesh={(m) => { currentMeshRef.current = m; setWallPositions(null); }}
        computingWall={computingWall}
        wallStats={wallStats}
        ...
        onClearWallMap={() => { setWallColors(null); setWallPositions(null); setWallStats(null); setSegStats(null); setWallThicknessArr(null); setWallFatArr(null); setFatStats(null); setCorridorResult(null); }}
        onComputeWallMap={async () => {
          const mesh = currentMeshRef.current;
          if (!mesh || mesh.triangleCount === 0) { setError('Build mesh first.'); return; }
          setComputingWall(true);
          setError(null);
          try {
            await new Promise((r) => setTimeout(r, 0));
            const res = computeWallThickness(mesh, volumeId, {
              stepMm: 0.4,
              maxMm: 15,
              contrastHU: 250,
              fatHU: 0,
            ...
            const n = res.totalVertexCount;
            const colors = new Float32Array(n * 3);
            const displaced = new Float32Array(mesh.positions.length);
            const meanT = Number.isNaN(res.meanMm) ? 5 : res.meanMm;
            for (let v = 0; v < n; v++) {
              const t = res.perVertex[v];
              const c0 = v * 3;
              if (Number.isNaN(t)) { colors[c0] = 0.4; colors[c0 + 1] = 0.4; colors[c0 + 2] = 0.4; }
              else {
                const [r, g, b] = lvThicknessToColor(t);
              ...
              const offset = (Number.isNaN(t) ? meanT : t) * layerDepth;
              const p0 = v * 3;
              displaced[p0]     = mesh.positions[p0]     + mesh.normals[p0]     * offset;
              displaced[p0 + 1] = mesh.positions[p0 + 1] + mesh.normals[p0 + 1] * offset;
              displaced[p0 + 2] = mesh.positions[p0 + 2] + mesh.normals[p0 + 2] * offset;
            }
            // Compute per-vertex fat fraction (HU < −30 within wall ray)
            const fatArr = computeFatFraction(mesh, res.perVertex, volumeId, {
              stepMm: 0.5, fatThreshold: -30, fatFloorHU: -190,
            });
            let fatSum = 0, fatCnt = 0, fatMax = 0, withFat = 0;
            for (let v = 0; v < n; v++) {
              const f = fatArr[v];
            ...
            setWallFatArr(fatArr);
            setFatStats(fatCnt > 0 ? {
              meanPct: (fatSum / fatCnt) * 100,
              maxPct: fatMax * 100,
              vertexWithFatPct: (withFat / fatCnt) * 100,
            } : null);

            // Border-zone corridor detection (VT conducting channels)
            const corridors = computeBZCorridors(mesh, res.perVertex, {
              scarMaxMm: 3, bzMaxMm: 5, weldTolMm: 0.3, minCorridorVerts: 30,
            });
            setCorridorResult(corridors);
            setWallColors(colors);
            setWallThicknessArr(res.perVertex);
            setWallPositions(displaced);
            const longAxis = (mvPoints.length === 3
              ? computeLongAxisFromMV(mvPoints, mesh, flipMV)
              : null) ?? computeLongAxisPCA(mesh);
            if (longAxis) {
              setSegStats(computeSegmentStats(mesh, res.perVertex, longAxis));
            } else {
              setSegStats(null);
            }
            setWallStats({
              ...
              maxMm: res.maxMm,
              meanMm: res.meanMm,
              measurable: res.measurableCount,
              total: res.totalVertexCount,
            });
          } catch (e: any) {
            setError(`Wall map failed: ${e?.message || e}`);
          } finally {
            setComputingWall(false);
          }
        }}
      />
      ...
      {hasMask && (
        <>
          <div className="la-section">
            <h4>4. Clean-up (keep LV cavity)</h4>
            <p className="la-hint">
              <strong>Goal:</strong> keep LV blood pool only. Remove LA via basal cut (step 5) and aorta via outflow cut. Erase brush for trabecular leakage into RV.
            </p>
            <div className="la-preset-row">
              <button className="la-btn la-btn-secondary" onClick={async () => {
                ...
                try {
                  ...
                  if (before === after) {
                    setStatusMsg('Mask already one blob. No fragments to drop. LV+aorta+LA stay connected — use Basal Cut + Outflow Cut to separate.');
                  } else {
                    await applyData(iso);
                ...
                <span className="la-btn-sub">keep largest blob (LV cavity)</span>
              </button>
              <button className="la-btn la-btn-secondary" onClick={async () => {
                const cur = laStateRef.current; if (!cur) return;
                setRunning(true);
          ...
          <div className="la-section">
            <h4>5. Basal Cut (MV annulus plane)</h4>
            <p className="la-hint">
              Click 3 points on the MV annulus (where LV meets LA). Plane removes LA side; LV cavity preserved (seed-anchored).
            </p>
      ...
      {segStats && (
        <div className="la-section">
          <h4>AHA 17-Segment Wall Thickness</h4>
          <p className="la-hint">
            Per-segment mean thickness (mm). Long-axis from {mvPoints.length === 3 ? 'MV plane' : 'PCA'}.
            Segments 1-6 basal, 7-12 mid, 13-16 apical, 17 apex.
          </p>
          <Bullseye17 segments={segStats} size={280} />
          <div style={{ fontSize: 10, color: '#9cb5d1', marginTop: 6, lineHeight: 1.5 }}>
            <strong>Anatomy:</strong> 1 ant · 2 antsep · 3 infsep · 4 inf · 5 inflat · 6 antlat<br />
            7-12 mid · 13 ant · 14 sep · 15 inf · 16 lat · 17 apex
          </div>
        </div>
      )}
