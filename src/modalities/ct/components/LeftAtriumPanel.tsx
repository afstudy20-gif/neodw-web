...
export interface LeftAtriumPanelHandle {
  saveSession: () => void;
      ...
      {visible && hasMask && data && dims && voxelToWorld ? (
        <>
          ...
          <div style={{ position: 'absolute', top: 6, right: 6, zIndex: 60, display: 'flex', gap: 4 }}>
            <button
              onClick={onRebuild}
              title="Rebuild LA mesh from current mask"
              style={{
        ...
        <div style={{
          ...
          {hasMask ? (
            ...
            <div>
              LA 3D mesh will appear here.<br />
...
const LA_EXCLUDE_SEGMENTATION_ID = 'leftAtriumExcludeMask';
...
export const LeftAtriumPanel = forwardRef<LeftAtriumPanelHandle, Props>(function LeftAtriumPanel(
  ...
  const clearSegmentation = useCallback(() => {
    ...
    if (excludeLabelmapIdRef.current) {
      for (const vpId of MPR_VIEWPORT_IDS) {
        try { segmentation.removeSegmentationRepresentations(vpId, { segmentationId: LA_EXCLUDE_SEGMENTATION_ID }); } catch { /* ignore */ }
      }
      try { segmentation.removeSegmentation(LA_EXCLUDE_SEGMENTATION_ID); } catch { /* ignore */ }
  ...
  const syncExcludeVisualization = useCallback(async () => {
    ...
    if (!excludeLabelmapIdRef.current) {
      ...
      segmentation.addSegmentations([
        {
          segmentationId: LA_EXCLUDE_SEGMENTATION_ID,
          representation: {
            type: ToolsEnums.SegmentationRepresentations.Labelmap,
            data: { volumeId: id },
          },
      ...
      if (lm && arr) {
        ...
        try {
          (cornerstoneTools as any).segmentation?.triggerSegmentationEvents
            ?.triggerSegmentationDataModified?.(LA_EXCLUDE_SEGMENTATION_ID);
        } catch { /* ignore */ }
      }
    }
  }, [volumeId]);
  ...
  const clearExcludeVisualization = useCallback(() => {
    ...
    if (excludeLabelmapIdRef.current) {
      for (const vpId of MPR_VIEWPORT_IDS) {
        try {
          segmentation.removeSegmentationRepresentations(vpId, { segmentationId: LA_EXCLUDE_SEGMENTATION_ID });
        } catch { /* ignore */ }
      }
      try { segmentation.removeSegmentation(LA_EXCLUDE_SEGMENTATION_ID); } catch { /* ignore */ }
      try { cornerstone.cache.removeVolumeLoadObject(excludeLabelmapIdRef.current); } catch { /* ignore */ }
  ...
  const applyData = useCallback(async (newData: Uint8Array, opts?: { skipUndo?: boolean }) => {
    ...
    if (lm && lmArr) {
      ...
      try {
        (cornerstoneTools as any).segmentation?.triggerSegmentationEvents
          ?.triggerSegmentationDataModified?.(LA_SEGMENTATION_ID);
      } catch { /* best-effort */ }
    } else {
  ...
  useEffect(() => {
    ...
    for (const vpId of MPR_VIEWPORT_IDS) {
      ...
      const paintAt = (clientX: number, clientY: number) => {
        const cur = laStateRef.current;
        if (!cur) { console.warn('[LA Paint] no mask — run Flood-Fill first'); return; }
        const rect = el.getBoundingClientRect();
        const cx = clientX - rect.left;
        const cy = clientY - rect.top;
  ...
  const saveSession = useCallback(() => {
    ...
    const fname = buildSessionFilename(patientName, studyDate);
  ...
  return (
    <div className="la-panel">
      ...
      <div className="la-section">
        ...
        <div className="la-preset-row">
          <button
            ...
            disabled={running}
            title="Click mid-chest axial slice inside LA contrast chamber (bright oval behind LV). HU target ≈300–400."
          >
          ...
          <button
            ...
            disabled={running}
            title="Add extra seeds for disconnected PV branches / LAA (multi-seed merge)."
      ...
      {hasMask && (
        <>
          <div className="la-section">
            <h4>4. Clean-up (keep LA + PVs)</h4>
            <p className="la-hint">
              <strong>Goal:</strong> keep LA body + 4 PV ostia (RSPV, RIPV, LSPV, LIPV). Remove LV (step 5 MV cut), aortic root (step 5b aorta cut), spine/IVC/SVC bleeds (Erase brush).
            </p>
            <div className="la-preset-row">
              <button className="la-btn la-btn-secondary" onClick={async () => {
                ...
                <span className="la-btn-sub">keep largest blob (LA body)</span>
              </button>
              <button className="la-btn la-btn-secondary" onClick={async () => {
                const cur = laStateRef.current; if (!cur) return;
                setRunning(true);
          ...
          <div className="la-section">
            <h4>5b. Aorta / Root Cut</h4>
            <p className="la-hint">
              Click 3 points on the aortic-root side of the LA/aorta bleed (typically anterior-medial above MV level). Plane removes aorta side; LA-side kept (seed-anchored). Use only if flood-fill leaked into aortic root through partial-volume contact.
            </p>
            <p className="la-hint" style={{ color: '#ffcc66' }}>
            ...
            >
              {aortaMode
                ? `Click point ${aortaPoints.length + 1}/3 on aortic root…`
                : aortaPoints.length >= 3
                  ? 'Re-pick Aorta Points'
